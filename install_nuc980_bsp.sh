...
if [ -d $letter/nuc980bsp ];then
        echo ''$letter'/nuc980bsp/ existed, (o)verwite, (r)emove or (a)bort?[y/n]';
        read remove;
        case "$remove" in
        [oO] )
                echo "Overwrite $letter/nuc980bsp folder"
                ;;
        [rR] )
                echo "Remove $letter/nuc980bsp folder"
                rm -rf $letter/nuc980bsp/*;
                ;;
        [aA] )
...
mkdir -p $letter/nuc980bsp/rootfs
tar -x -f ${BINARIES_DIR}/rootfs.tar --directory=$letter/nuc980bsp/rootfs
if grep -q "BR2_PACKAGE_APPLICATIONS=y" $BR2_CONFIG; then  \
	cp ${BUILD_DIR}/$APPLICATION_VER $letter/nuc980bsp/application -r & \
fi
if grep -q "BR2_TARGET_UBOOT=y" $BR2_CONFIG; then  \
	cp ${BUILD_DIR}/$UBOOT_VER $letter/nuc980bsp/uboot -r & \
fi
pid=`echo $!`
str="| / - \\"
count=1
...
mkdir -p $letter/nuc980bsp/image
cp ${BUILD_DIR}/image/* $letter/nuc980bsp/image/ -a
cp ${BINARIES_DIR}/* $letter/nuc980bsp/image/ -a
if grep -q "BR2_LINUX_KERNEL=y" $BR2_CONFIG; then  \
	cp ${BUILD_DIR}/$KERNEL_VER $letter/nuc980bsp/linux-4.4.x -r &
fi
pid=`echo $!`
str="| / - \\"
count=1
...
if [ -z $SUDO_USER ];then
        ...
        chown -R $SUDO_USER:$SUDO_USER $letter/nuc980bsp;
fi
